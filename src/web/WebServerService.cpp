...
void WebServerService::routes() {
  ...
  server_.on("/api/system/status",HTTP_GET,[this](){ if(!authorized()){sendJson(401,"{\"error\":\"unauthorized\"}");return;} DynamicJsonDocument d(3072); d["name"]=NAME;d["version"]=VERSION;d["deviceName"]=config_.get().deviceName;d["uptimeMs"]=millis();d["heapFree"]=ESP.getFreeHeap();d["heapMin"]=ESP.getMinFreeHeap();d["psramTotal"]=ESP.getPsramSize();d["psramFree"]=ESP.getFreePsram();d["batteryPercent"]=power_.batteryPercent();d["batteryMv"]=power_.batteryMillivolts();d["sdMounted"]=storage_.available();d["sdFree"]=storage_.freeBytes();d["ip"]=wifi_.ip().toString();d["mac"]=WiFi.macAddress(); String o;serializeJson(d,o);sendJson(200,o); });
  server_.on("/api/battery",HTTP_GET,[this](){ if(!authorized()){sendJson(401,"{\"error\":\"unauthorized\"}");return;} sendJson(200,String("{\"percent\":")+power_.batteryPercent()+",\"millivolts\":"+power_.batteryMillivolts()+"}"); });
  ...
  server_.on("/api/settings",HTTP_GET,[this](){ if(!authorized()){sendJson(401,"{\"error\":\"unauthorized\"}");return;} DynamicJsonDocument d(1024);d["deviceName"]=config_.get().deviceName;d["language"]=config_.get().language;d["timezone"]=config_.get().timezone;d["sleepMinutes"]=config_.get().sleepMinutes; String o;serializeJson(d,o);sendJson(200,o); });
  server_.on("/api/settings",HTTP_POST,[this](){ if(!authorized()){sendJson(401,"{\"error\":\"unauthorized\"}");return;} DynamicJsonDocument d(2048);if(deserializeJson(d,body())){sendJson(400,"{\"error\":\"invalid_json\"}");return;} if(d.containsKey("deviceName"))config_.edit().deviceName=String((const char*)d["deviceName"]);if(d.containsKey("language"))config_.edit().language=String((const char*)d["language"]);if(d.containsKey("timezone"))config_.edit().timezone=String((const char*)d["timezone"]);if(d.containsKey("sleepMinutes"))config_.edit().sleepMinutes=d["sleepMinutes"].as<uint32_t>();bool ok=config_.save();sendJson(ok?200:500,ok?"{\"ok\":true}":"{\"ok\":false}"); });
