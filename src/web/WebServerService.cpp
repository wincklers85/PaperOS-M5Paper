...
<body><main><div class="brand"><span class="mark">P</span>PaperOS <span style="font-weight:500;color:#69716c">/ SD Recovery</span></div><div class="eyebrow">READ-ONLY SECTOR SCAN</div><h1>Deleted files</h1><p>PaperOS scans the card sector by sector for JPEG, PNG, BMP, PDF and readable text signatures, while also checking FAT32 deleted entries.</p><div class="warning"><b>Recovery limits:</b> scans may take hours. Raw carving cannot restore original names or folders and assumes each recovered file is contiguous; fragmented, overwritten or encrypted content may be damaged or missed. NTFS/APFS metadata recovery is not included. Saving back onto this SD can overwrite deleted data.</div><div class="toolbar"><button id="scanBtn" onclick="scan()">Scan SD card</button><button id="cancelBtn" class="secondary" onclick="cancelScan()" style="display:none">Stop scan</button><progress id="progress" max="100" value="0" style="width:min(240px,100%);height:14px"></progress><span id="status" class="status">Loading scan results…</span></div><div id="files" class="grid"></div></main>
<script>
let pollTimer=null;
function updateScanUi(d){document.getElementById('status').textContent=d.status;document.getElementById('progress').value=d.progress||0;document.getElementById('scanBtn').textContent=d.scanning?'Scanning…':'Scan SD card';document.getElementById('scanBtn').disabled=!!d.scanning;document.getElementById('cancelBtn').style.display=d.scanning?'inline-block':'none'}
async function scan(){try{let r=await fetch('/api/recovery/scan',{method:'POST'});let d=await r.json();updateScanUi(d);if(d.scanning)pollScan();else await load()}catch(e){document.getElementById('status').textContent='Scan failed. Keep the device powered on and retry.'}}
async function cancelScan(){try{await fetch('/api/recovery/cancel',{method:'POST'});if(pollTimer)clearTimeout(pollTimer);await load()}catch(e){document.getElementById('status').textContent='Could not stop the scan.'}}
async function pollScan(){try{let r=await fetch('/api/recovery/files');let d=await r.json();updateScanUi(d);if(d.scanning)pollTimer=setTimeout(pollScan,1200);else await load()}catch(e){pollTimer=setTimeout(pollScan,2500)}}
async function load(){try{let r=await fetch('/api/recovery/files');let d=await r.json();updateScanUi(d);let root=document.getElementById('files');root.replaceChildren();if(!d.files.length){root.innerHTML='<div class="empty">No candidates found yet. A complete raw scan may take hours; you can stop it and keep candidates already found.</div>';if(d.scanning)pollScan();return}for(let f of d.files){let el=document.createElement('article');el.className='file';let title=document.createElement('h2');title.textContent=f.name;let meta=document.createElement('div');meta.className='meta';meta.textContent=(f.size/1024).toFixed(1)+' KB · '+(f.raw?(f.freeClusterVerified?'raw carve · free FAT cluster':'raw signature match · deletion unverified'):'FAT32 deleted entry')+' · '+(f.preview?'Preview available':'download only');let prev=document.createElement('div');prev.className='preview';if(f.kind==='image'&&f.size<=2097152){let im=document.createElement('img');im.loading='lazy';im.src='/api/recovery/preview?index='+f.index;prev.append(im)}else if(f.kind==='image'){prev.textContent='Image preview is limited to 2 MiB; download to inspect this candidate'}else if(f.kind==='text'){let pre=document.createElement('pre');let tx=await fetch('/api/recovery/preview?index='+f.index);pre.textContent=await tx.text();prev.append(pre)}else{prev.textContent='No preview for this file type'}let actions=document.createElement('div');actions.className='actions';let dl=document.createElement('button');dl.textContent='Recover to this device';dl.onclick=()=>{if(confirm('Recovery can be incomplete if sectors were reused or the file was fragmented. This downloads the candidate to this device without writing to the SD. Continue?'))location.href='/api/recovery/download?index='+f.index};let sd=document.createElement('button');sd.className='secondary';sd.textContent='Save onto this SD';sd.disabled=!!d.scanning||f.size>1048576;sd.title=d.scanning?'Stop the read-only scan first':(sd.disabled?'Same-card restore is limited to 1 MiB; export to another device instead':'');sd.onclick=()=>saveToSource(f);actions.append(dl,sd);el.append(title,meta,prev,actions);root.append(el)}if(d.scanning)pollScan()}catch(e){document.getElementById('status').textContent='Sign in to Web Console, then reload this page.'}}
async function saveToSource(f){const risk='This writes to the SOURCE SD card. Even though PaperOS first stages this file in memory, writing may overwrite other deleted files and make them unrecoverable. Use another card or download to this device when possible. Continue?';if(!confirm(risk))return;if(!confirm('Final confirmation: write '+f.name+' ('+(f.size/1024).toFixed(1)+' KB) to /PaperOS/Recovery on this same SD?'))return;document.getElementById('status').textContent='Staging the full file, then writing to the SD…';try{const u='/api/recovery/save?index='+encodeURIComponent(f.index)+'&confirm=SOURCE_SD_OVERWRITE_RISK_ACCEPTED';const r=await fetch(u,{method:'POST'});const d=await r.json();document.getElementById('status').textContent=d.destination?d.status+' Destination: '+d.destination:d.status;if(d.ok)await load()}catch(e){document.getElementById('status').textContent='Save failed. Check the SD card and retry.'}}
load();
</script></body></html>
)paperosrecover";
...
void WebServerService::routes() {
  ...
  server_.on("/api/recovery/scan",HTTP_POST,[this](){ if(!authorized()){sendJson(401,"{\"error\":\"unauthorized\"}");return;} bool ok=storage_.startRecoveryScan(); DynamicJsonDocument d(768); d["ok"]=ok; d["scanning"]=storage_.recoveryScanning(); d["progress"]=storage_.recoveryProgress(); d["status"]=storage_.recoveryStatus(); d["count"]=storage_.recoveredFileCount(); String o; serializeJson(d,o); sendJson(ok?202:415,o); });
  server_.on("/api/recovery/cancel",HTTP_POST,[this](){ if(!authorized()){sendJson(401,"{\"error\":\"unauthorized\"}");return;} storage_.cancelRecoveryScan(); sendJson(200,"{\"ok\":true}"); });
  server_.on("/api/recovery/files",HTTP_GET,[this](){
    if(!authorized()){sendJson(401,"{\"error\":\"unauthorized\"}");return;}
    DynamicJsonDocument d(24576); d["status"]=storage_.recoveryStatus(); d["scanning"]=storage_.recoveryScanning(); d["progress"]=storage_.recoveryProgress(); JsonArray files=d.createNestedArray("files");
    for(size_t i=0;i<storage_.recoveredFileCount();++i){
      ...
      x["kind"]=kind; x["preview"]=kind!="other"; x["raw"]=f->rawCarved; x["freeClusterVerified"]=f->rawDeletedSpaceVerified;
    }
  ...
  server_.on("/api/recovery/save",HTTP_POST,[this](){
    if(!authorized()){sendJson(401,"{\"error\":\"unauthorized\"}");return;}
    if(server_.arg("confirm")!="SOURCE_SD_OVERWRITE_RISK_ACCEPTED"){
      sendJson(400,"{\"ok\":false,\"status\":\"Two explicit confirmations in the recovery app are required before writing to the source SD\"}");return;
    }
    int index=server_.arg("index").toInt(); String destination,status;
    bool ok=index>=0 && storage_.saveRecoveredFileToSource(index,true,destination,status);
    DynamicJsonDocument d(512); d["ok"]=ok; d["status"]=status.length()?status:"Invalid recovery candidate"; if(destination.length())d["destination"]=destination;
    String o; serializeJson(d,o); sendJson(ok?200:400,o);
  });
  ...
  server_.on("/api/system/status",HTTP_GET,[this](){ if(!authorized()){sendJson(401,"{\"error\":\"unauthorized\"}");return;} DynamicJsonDocument d(3584); const uint32_t psramUsable=ESP.getPsramSize(); d["name"]=NAME;d["version"]=VERSION;d["deviceName"]=config_.get().deviceName;d["uptimeMs"]=millis();d["heapFree"]=ESP.getFreeHeap();d["heapMin"]=ESP.getMinFreeHeap();d["psramTotal"]=psramUsable;d["psramUsableBytes"]=psramUsable;d["psramInstalledBytes"]=PSRAM_INSTALLED_BYTES;d["psramFree"]=ESP.getFreePsram();d["batteryPercent"]=power_.batteryPercent();d["batteryMv"]=power_.batteryMillivolts();d["batteryTrend"]=power_.batteryTrendLabel();d["chargingLikely"]=power_.chargingLikely();d["batteryCurrentSupported"]=power_.batteryCurrentSupported();d["wakeReason"]=power_.wakeReason();d["sdMounted"]=storage_.available();d["sdFree"]=storage_.freeBytes();d["ip"]=wifi_.ip().toString();d["mac"]=WiFi.macAddress(); String o;serializeJson(d,o);sendJson(200,o); });
  ...
  server_.on("/api/notes/item",HTTP_POST,[this](){ if(!authorized()){sendJson(401,"{\"error\":\"unauthorized\"}");return;} if(storage_.recoveryScanning()){sendJson(423,"{\"error\":\"recovery_scan_active\"}");return;} String id=server_.arg("id");bool ok=notes_.save(id,body());sendJson(ok?200:400,ok?"{\"ok\":true}":"{\"ok\":false}"); });
  server_.on("/api/notes/delete",HTTP_POST,[this](){ if(!authorized()){sendJson(401,"{\"error\":\"unauthorized\"}");return;} if(storage_.recoveryScanning()){sendJson(423,"{\"error\":\"recovery_scan_active\"}");return;} DynamicJsonDocument d(512);deserializeJson(d,body());bool ok=notes_.remove(d["id"]|"");sendJson(ok?200:400,ok?"{\"ok\":true}":"{\"ok\":false}"); });
